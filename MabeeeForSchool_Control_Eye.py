...
# MaBeee for School (Scratch版) 専用アプリ - クリーン版(ログ出力なし)
class App:
    ...
    def load_config(self):
        if os.path.exists(self.config_file):
            ...
            self.config['Settings'] = {'sound': 'True', 'mode': '1', 'size': '中', 'timer': '5', 'camera': 'カメラなし'}
            self.save_config()
    ...
    def setup_ui(self):
        ...
        adm = tk.LabelFrame(self.header, text=" 【MaBeee for School専用】設定 ", font=f_b, bg="white", fg="#D32F2F", padx=15, pady=10)
        ...
        self.lbl_esc = tk.Label(self.root, text="※[F11]キー:全画面表示切替 / [1][2][3]キー:動作モード切替 / [←][→]キー:タイマー設定", font=f_b, bg="#f0f0f0", fg="#555")
    ...
    async def do_connect(self, silent=False):
        ...
        try:
            if not silent: self.up_s("接続中...", "orange")
            if self.client:
            ...
            f_uuid = None
            self.scratch_uuids = []
            self.current_on = None # 接続時にリセット
            
            for attempt in range(3):
                if attempt > 0: 
                    self.up_s(f"接続中... ({attempt+1}回目)", "blue")
            ...
            else: self.up_s("窓口不明", "red")
        except Exception: self.up_s("接続失敗", "red")
            ...
            try:
                ...
                for d in ds:
                    ...
                    if "scratch" in name or "mabee" in name or "mb" in name or d.name:
                        ...
                        disp = d.name if d.name and d.name.strip() else f"名前なし({d.address})"
                        info.append((rssi, d, disp))
                info.sort(key=lambda x: x[0], reverse=True)
                for r, dev, dname in info:
                    found.append(dev)
                    addr = dev.address
                    if r != -100: nms.append(f"{dname} (電波: {r}dBm) [{addr}]")
...
if __name__ == "__main__":
    # Windowsにおける二重起動防止のチェック
    if sys.platform == 'win32':
        import ctypes
        kernel32 = ctypes.windll.kernel32
        # ガベージコレクションを避けるため変数に保持する
        _app_mutex = kernel32.CreateMutexW(None, True, "Global\\MabeeeForSchool_Control_Eye_Mutex_Unique")
        if kernel32.GetLastError() == 183:  # ERROR_ALREADY_EXISTS
            # すでに起動している場合は警告を表示して終了
            ctypes.windll.user32.MessageBoxW(
                0,
                "MabeeeForSchool Control Eye アプリは既に起動しています。画面が表示されるまでしばらくお待ちください。",
                "二重起動防止",
                0x00000030  # MB_ICONWARNING
            )
            sys.exit(0)

    # Windowsでasyncioを使用する場合のポリシー設定
    if sys.platform == 'win32':
